...
export function AudiencePlans() {
  return (
    <section className="border-y border-white/10 bg-med-panel/60" id="planes">
      <div className="mx-auto w-full max-w-7xl px-5 py-16 md:px-8">
        ...
        <div className="grid gap-5 lg:grid-cols-[1fr_1.6fr]">
          ...
          >
            <h3 className="text-lg font-black text-med-text">Público objetivo</h3>
            <ul className="mt-3 space-y-2">
              {audience.map((item) => (
                <li key={item} className="rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-sm text-med-muted">
                  {item}
                </li>
              ))}
            </ul>
          ...
          <div className="grid gap-3 md:grid-cols-3">
            {plans.map((plan, idx) => (
              <motion.article
                ...
                className="rounded-2xl border border-white/10 bg-med-card p-5"
              >
                <h3 className="text-lg font-black text-med-text">{plan.name}</h3>
                <p className="mt-2 min-h-20 text-sm text-med-muted">{plan.description}</p>
                <ul className="space-y-1.5">
                  {plan.points.map((point) => (
                    <li key={point} className="text-xs text-white/75">
                      • {point}
                    </li>
                  ))}
                </ul>
                <a
                  href="#contacto"
                  className="mt-4 inline-flex rounded-lg border border-med-blue/35 bg-med-blue/10 px-3 py-2 text-xs font-black uppercase tracking-wider text-med-blue transition hover:bg-med-blue/20"
                >
                  {plan.cta}
                </a>
              </motion.article>
