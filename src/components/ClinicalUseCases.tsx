...
export function ClinicalUseCases() {
  return (
    <section className="mx-auto w-full max-w-7xl px-5 py-16 md:px-8" id="casos">
      <SectionTitle
        ...
        {cases.map((item, idx) => (
          ...
          >
            <div className="flex items-center justify-between gap-3">
              <h3 className="text-lg font-black text-med-text">{item.name}</h3>
              <span className="rounded-full border border-med-ecg/30 bg-med-ecg/10 px-2 py-1 text-[10px] font-black uppercase tracking-wider text-med-ecg">
                ACLS
              </span>
            </div>
            <p className="mt-2 text-sm text-med-muted">{item.focus}</p>
          </motion.article>
        ))}
      </div>
    </section>
